...
export async function getAllAExpenses(userId) {
    try {
        const expenses = await db
            .select()
            .from(transactionsTable)
            .where(and(eq(transactionsTable.userId, userId), eq(transactionsTable.transactionType, TRANSACTION_TYPES.EXPENSE)))
...
export async function getExpenses(userId) {
    try {
        const expenses = await db
            .select()
            .from(transactionsTable)
            .where(and(eq(transactionsTable.userId, userId), eq(transactionsTable.transactionType, TRANSACTION_TYPES.EXPENSE), isNull(transactionsTable.referenceId)));
        return expenses;
    }
    catch (error) {
        throw new Error("Failed to retrieve expenses: " + error);
...
export async function getCategoriesExpenses(userId) {
    try {
...
export async function getCategoriesExpensesById(userId, categoryId) {
    try {
        const categoryExpenses = await db
            .select()
            .from(transactionsTable)
            .where(and(eq(transactionsTable.userId, userId), eq(transactionsTable.transactionType, TRANSACTION_TYPES.EXPENSE), eq(transactionsTable.referenceId, categoryId)))
            .orderBy(desc(transactionsTable.createdAt));
...
export async function createExpense(userId, expenseData) {
    try {
        const { amount, description, referenceId, categoryId, accountId } = expenseData;
        if (accountId) {
...
export async function deleteExpense(userId, expenseId) {
    try {
        const deletedExpense = await db
            ...
            .where(and(eq(transactionsTable.id, expenseId), eq(transactionsTable.userId, userId)))
            .returning();
...
export async function createExpenseCategory(userId, categoryData) {
    try {
...
export async function deleteExpenseCategory(userId, categoryId) {
    try {
...
export async function addExpenseToCategory(userId, categoryId, expenseData) {
    try {
