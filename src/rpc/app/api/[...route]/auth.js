...
function createToken(user) {
    return jwt.sign({
        ...
        email: user.email,
        name: user.name,
    }, JWT_SECRET, { expiresIn: "5y" });
}
const authRoute = new Hono().post("/firebase", zValidator("form", authValidator), async (c) => {
    try {
        ...
        return c.json(Responses.success("Successfully authenticated", {
            id: user.id,
            name: user.name,
            email: user.email,
